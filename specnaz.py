...
# Ostatnia Aktualizacja: 19/02/2023 - version 1.2
...
def menu():
    ...
    print(f"""
        ...
        {w}{b}  05{w} Infofon      {d} Phone number information
...
def mainmenu():
    while True:
        try:
            cmd = str(input(f"{space}{w}{b}>{w} Wybierz:{b} "))
            if int(len(cmd)) < 6:
...
def infoga(opt):
    x = str(input(f"{space}{b}>{w} Wpisz domenę lub adres IP:{b} "))
    if not x: menu()
...
def phoneinfo():
    no = str(input(f"{space}{b}>{w} Wpisz numer:{b} "))
    if not no: menu()
    print(w+lines)
